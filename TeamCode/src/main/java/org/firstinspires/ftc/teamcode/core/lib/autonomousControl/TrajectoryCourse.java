...
public class TrajectoryCourse implements TrajectoryStructure{
    ...
    public boolean execute(Pose2d currentBotPosition,RobotMovementState currentBotState,double elapsedTime){
        ...
        XDrive.getInstance().controlBasedOnVelocity(vData,elapsedTime);

        return (CurrentIndex==xPointList.size()-1)&&
                (Math.abs(XDrive.getInstance().getCurrentPose().getHeadingDegrees() - pose2dList.get(getSegmentID()).getHeadingDegrees())) <= AutonomousConstants.TOLERATED_HEADING_ERROR;
